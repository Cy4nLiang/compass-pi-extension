...
持久化（`store.ts` 的 CompassRepository）：单一 JSON store（schemaVersion 1，load 时 assertStore 校验）写入**宿主项目**（运行 pi 的 cwd）的 `.pi/compass/`，而不是扩展自身目录。新增顶层集合一律走「可选数组 + `ensureDefaults` 回填 + `load()` 迁移检测与回写」（`outcomeChecks` / `lessons` / `todoResolutions` / `costReferences` 是先例），schemaVersion 保持 1，旧版扩展回滚后忽略新字段即可打开。写入走临时文件 + rename 原子替换，目录 0700、文件 0600。`resolveInputPath` / `resolveOutputPath` 把一切输入输出路径限制在宿主项目内——不要绕过它们直接拼路径。
...
A 档补数（`compass_gaps approve` / `convert` + `gapfill-convert.ts`）另有五条不变式：

- **参考成本单（`kind: "cost_reference"`）只在显式 `origin=purchase_cost_source` 时入选**：新缺口「采购价出处」对所有手填测算的市场都会出（owner 2026-09-07 拍板不限范围），任何这样的市场都同时挂着快照链（`import_csv`）与参考成本链（`profit_estimate`）两条 A 档路由，approve 若不在 `origin` 缺省时排除后者，「A 档产物唯一」那道检查会把运营照旧只写 `market_ref` 的 approve 一律拒绝。这条链的「完整」= 1 份本关键词的检索载荷 + ≥ 1 条运营在 TUI 逐条确认纳入的样本；不产 CSV、不产材料，产物是 `store.costReferences` 里的一条记录（可同事务克隆最新测算写回一条出处为 `ali1688_reference` 的利润测算）。同款确认循环与最终确认弹窗都在 `mutateStore` **之外**、上界 `COST_REFERENCE_MAX_PROMPTS`，超时 / Esc 只中止不清单；克隆最新测算要在 mutator 内用事务里的 store 重取（循环可长达 12 分钟）。服务端「无结果」是纯文本哨兵（映射表 `empty_sentinel`），必须在 `JSON.parse` 之前比对判成正常空值并归档原文，否则会走「返回体不是 JSON、那一步要重调」的劝烧钱文案。strict 档按 `search_name` 复核、`page` 只认 1，固定参数比对两边 `String()`（page 在 schema 里是整数）。汇率表 `.pi/gapfill/fx.json` 与映射表同样住在宿主工作区、缺失或无效一律抛错不降级，且在 approve 花钱之前就校验。`tests/static-invariants.test.ts` 的四条「参考成本」切片钉子守着以上位置。
...
- **归池口径 = adapter 的「展开 + 分派」规则**（N-AUD-4）：`mcpCallTargetServers` 回答「这次调用打向哪个池」，是**熔断门、补数确认单预扣、在途预占**三处共用的第一句判定（三处都以 `if (!servers.length) return undefined` 早退），所以它漏一种形态等于三道门同时失效，而计量侧照常记账——钱花了、门没拦。除顶层 `server` / `tool` 外还必须认「网关参数被套进 `args` 里」的兼容形态：adapter 在顶层七个网关键（`tool` / `connect` / `describe` / `instructions` / `search` / `server` / `action`；判据是 `!== undefined`，**空串也算出现**）一个都没出现时，会把 `args` 里的参数展开成 `dispatchParams` 再走分派链，`args` 可以是对象也可以是 JSON 字符串，且**只展开一层**。归池只认**会落到 `executeCall` 的形态**（`nestedGatewayCallTarget`）：三个已知 `action` 抢在 `tool` 之前分派、`tool` 是空串或缺席、adapter 会当场抛错的形态，一律不归——`args` 被刻意排除在上一条的非调用白名单之外，嵌套侧一旦误归就**没有任何东西替它放行**，熔断后连 `describe` 都问不出来，等于把上一条刚修好的毛病原路种回去。两份键集方向相反、成员也不同（这里七键含 `tool` 不含 `regex` / `limit`，白名单反之），**不可互相复用**；两处 `action` 名单同值纯属巧合，一份问「会不会抢在 tool 之前分派」，另一份问「发不发请求」。
