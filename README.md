...
| `compass_gaps` | 汇总数据缺口并按成本档给出补数计划；`approve` 当面确认后授权付费补数（三条链：完整快照 / 差评材料 / 1688 参考成本），`convert` 把返回体确定性地转成产物——可导入的 CSV、差评材料文件，或在逐条确认同款后算出的参考采购价 |
| `compass_dispatch` | 派发零工具子代理：差评聚类、风险检索式生成、供应商询价模板；结果不写回，单会话有次数上限，受限会话不可用 |
...
**仅绑定 `127.0.0.1`，不做鉴权（本机单用户模型）：不要用端口转发把它暴露到局域网或公网**——工作台能读写全部选品经营数据。同一模型下，本机其它端口的页面可以跨源**读**只读接口（回环 Origin 的 GET 回 `Access-Control-Allow-Origin`）；写接口不开放跨源（不处理预检，且 Origin 仍要校验）。
...
- 不把缺失指标伪装成通过；
- 不把 AI 风险初筛当法律意见；
- 花钱的补数确认（`compass_gaps action=approve`）与 1688 逐条同款确认只在**能当面确认的会话**里放行：TUI，或宿主进程显式设置环境变量 `COMPASS_CONFIRM_HOST=chat` 的 rpc 会话（宿主替运营把确认弹窗画出来）。这是声明不是凭据——它保住的是「默认拒绝」：print / json 与未声明的 rpc（脚本、CI）一律拒绝；
- 强调采集环境与卖家主账号环境物理隔离；
...
- **上架日期**支持 ISO（含毫秒与时区）、`2024/1/5`、`2024.1.5`、`2024年1月5日` 与 Excel 序列号；纯日期一律按 UTC 零点解释、与运行机器时区无关，不存在的日期（2 月 30 日）按缺失处理；该列识别率低于 50% 时给出告警。
