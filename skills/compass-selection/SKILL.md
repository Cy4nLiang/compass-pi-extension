...
- `compass_gaps approve market_ref=…` 是**唯一会花钱的子命令**：它在 TUI 里弹一次确认，运营按下确认后发一张 10 分钟有效的确认单，授权按映射表声明的链路调用付费数据源补齐这个市场的完整快照。拿到返回后用 `compass_gaps convert market_ref=…` 把返回体确定性地转成一份市场 CSV（不经模型猜数字），再照它给出的命令 `compass_import_csv` 导入。
...
- convert 产出的 CSV 是全英文表头，导入时**必须显式写 `source=sorftime`**（否则会被识别成通用 CSV，在同一个市场里凭空造出「多来源」）；`captured_at` 用转换当天，因为这批数据就是此刻从线上取的。
...
- **补完数据不要为了「解释一下现在怎么样」重跑 `compass_strategy_run`**：它硬编码 `trigger="manual"`，会被记成对 P1「复盘 challenged」的人工处置并清掉那条告警。要看结果就看导入结果里的粗筛结论。
- 补数后不代运营 `compass_todo submit / verify / complete`，也不代 `compass_pool move / decide`。
- Sorftime 补数之后出现多来源指标偏差待办是**预期行为**（口径由运营定），不是补数失败。
