...
- `compass_gaps list` 给缺口清单（按成本档分组），`compass_gaps plan market_ref=…` 给某个市场的逐条补数计划。只读派生、不写库、不花钱。
- 三档分工：**C 档**（重导带列 CSV、查本地历史）可以直接做；**A 档**（付费数据源）本期只做提示，自动补数在下一期上线，不要向运营承诺现在就能自动补；**人工**（供应商报价、官方证据链接、店铺实绩、预估星级）只能由运营给。
...
- **补数缺口本身不花钱**：`compass_gaps` 只读派生，A 档的自动补数尚未上线，任何「我去补一下数据」都不得变成实际的付费调用。
