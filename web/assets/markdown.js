...
// 三方行为不一致的语法一律不用。
// 前端零构建、零运行时依赖的约定延伸到这里：不引第三方 Markdown 库，脚本与样式一律本地加载。
// 全站唯一的外链是 index.html 里那条 IBM Plex 字体 <link>，且已改成非阻塞形态
// （media="print" + onload 换回 all）：工作台只绑回环地址、必须能离线打开，字体拿不到时由
// style.css 的回退栈接管，功能与渲染结果都不受影响。
//
